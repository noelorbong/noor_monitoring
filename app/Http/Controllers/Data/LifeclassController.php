...
class LifeclassController extends Controller
{
    ...
    public function index()
    {
        ...
        ->select(
            'ct.*'
            ,DB::Raw('(select count(*) from members where  FIND_IN_SET(ct.id,members.lifeclass) > 0 ) as lifeclass_member')
    ...
    {    $default = DB::table('lifeclasses')
        ...
        if($default->counted <= 0){
            DB::table('lifeclasses')->insert(
                [
                'id' => 1,
                'lifeclass_name' => ' No lifeclasses',
                'description' => 'No lifeclasses'
                ]
            );
        }
