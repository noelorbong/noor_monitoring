<?php
namespace App\Http\Controllers\Data;

use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Facades\File;

class CategoryController extends Controller
{
      
    public function index()
    {
        $records = DB::table('categories as ct')
        ->select(
            'ct.*'
            ,DB::Raw('(select count(*) from members where  FIND_IN_SET(ct.id,members.category) > 0 ) as category_member')
            
           )
        ->orderByRaw('ct.category_name asc')
        ->get();

        return  compact('records');
    }

    public function store(Request $request)
    {    
        $default = DB::table('categories')
        ->select(
            DB::RAW('Count(*) as counted')     
           )
        ->where('id','=',1)
        ->first();

        if($default->counted <= 0){
            DB::table('categories')->insert(
                [
                    'id' => 1,
                'category_name' => ' No Category',
                'description' => 'No Category'
                ]
            );
        }

        DB::table('categories')->insert(
            [
            'category_name' => $request->category_name,
            'description' => $request->description
            ]
        );
        
        return 'success';
    }

     public function edit($id)
    {    
        
         $record = DB::table('categories')
        ->select(
            '*'     
           )
        ->where('id','=',$id)
        ->first();

        return  compact('record');
    }

    public function update(Request $request)
    {
        DB::table('categories')
        ->where('id','=',$request->id)
        ->update([
            'category_name' => $request->category_name,
            'description' => $request->description,
            'updated_at' => DB::Raw("NOW()")
            ]
        );
        
        return 'success';
    }

    public function delete($id)
    {
    
    DB::table('categories')->where('id', '=', $id)->delete();

    $users = DB::table('members')
            ->select(
                'members.*'
            )
            ->where(DB::raw('FIND_IN_SET('.$id.',members.category)') , '>', 0)
            ->get();

    foreach ($users as $user){

    
        $str_arr = explode (",", $user->category);
        $array1= array();

        foreach ($str_arr as $value){ 
            if($value != $id){
                array_push($array1,$value);
            } 
        } 

        if(count($array1) > 0){
            $new_category = join( ',', $array1);
        }else{
            $new_category = "1";
        }

        DB::table('members')
        ->where('id','=', $user->id)
        ->update(
                [
                'category' => $new_category,
                'updated_at' => DB::Raw("NOW()")
                ]
            );
    }
        return 'success';
    }

    public function remove($id,$category_id)
    {
        $user = DB::table('members')
            ->select(
                'members.*'
            )
            ->where('id','=',$id)
            ->first();

        $str_arr = explode (",", $user->category);
        $array1= array();

        foreach ($str_arr as $value){ 
            if($value != $category_id){
                array_push($array1,$value);
            } 
        } 

    if(count($array1) > 0){
            $new_category = join( ',', $array1);
        }else{
            $new_category = "1";
        }

    DB::table('members')
      ->where('id','=',$id)
      ->update(
            [
            'category' => $new_category,
            'updated_at' => DB::Raw("NOW()")
            ]
        );

        return 'success';
    }

    public function profile($id)
    {
        $user = DB::table('categories as ct')
        ->select(
            'ct.*'
             ,DB::Raw('(select count(*) from members where  FIND_IN_SET(ct.id,members.category) > 0 ) as category_member')
            
           )
        ->where('id','=',$id)
        ->first();

         $records = DB::table('members as mb')
        ->leftJoin('tribes', 'mb.tribe', '=', 'tribes.id')
        //->leftJoin('categories', 'mb.category', '=', 'categories.id')
        ->select('mb.*'
        ,'tribes.tribe_name'
        //,'categories.category_name'
        ,DB::raw('concat(COALESCE(mb.address1,""),", ",COALESCE(mb.address2,"")," ",COALESCE(mb.barangay,"")," ",COALESCE(mb.zipcode,"")," ",COALESCE(mb.municipality,"")," ",COALESCE(mb.province,"")) as complete_address ')
        ,DB::raw('concat(COALESCE(mb.last_name,""),", ",COALESCE(mb.first_name,"")," ",COALESCE(mb.name_extension,"")," ",COALESCE(mb.middle_name,"")) as full_name') 
        ,DB::raw('(Select concat(COALESCE(last_name,""),", ",COALESCE(first_name,"")," ",COALESCE(name_extension,"")," ",COALESCE(middle_name,"")) from members where id = mb.mentor limit 1) as mentor_name')
        )
       // ->whereRaw('FIND_IN_SET(2,sent_mail_ids) = 0')
        ->where(DB::Raw('FIND_IN_SET("'.$id.'",mb.category)'), '>', 0)
        ->orderByRaw('mb.last_name asc')
        ->get();

        return  compact('user','records');
    }

    public function memberUpdate(Request $request)
    {   
        $members = explode (",", $request->member_id); 

        foreach ($members as $key => $value){
            $category_id = "1";

            $user = DB::table('members')
            ->select(
                'members.*'
            )
            ->where('id','=',$value)
            ->first();

            if($user->category == "1"){
                $category_id = $request->category_id;
            }else{
                $category_id = $user->category.",".$request->category_id;
            }

             DB::table('members')
                ->where('id','=',$value)
                ->update(
                    [
                    'category' => $category_id,
                    ]
                );

        }
        
        return 'success';
    }

}