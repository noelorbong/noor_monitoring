      ...
      <div class="col-sm-4" v-for="(data) in searchResult" :key="data.id">
        <div class="card">
          <div class="card-header">
            ...
            {{ data.specializedministry_name || ''}}
          </div>
          <div class="card-body">
            <div>
              <span style="font-weight: bold;">Member:</span>
              {{ data.specializedministry_member || ''}}
              <br />
            </div>
            <p
              class="truncate"
          ...
          <div class="card-footer">
            ...
            <router-link
              :to="{name: 'SpecializedMinistryProfile', params: {id: data.id}}"
            ...
            <router-link
              v-if="data.id != 1"
              :to="{name: 'SpecializedMinistryEdit' , params: {id: data.id}}"
...
export default {
  ...
  created: function() {
    ...
    app.isLoading = true;
    axios
      .get("/auth/specializedministry/list")
      .then(function(resp) {
  ...
  computed: {
    searchResult: function() {
      ...
      return this.datas.filter(item => {
        return (
          (item.specializedministry_name + " " + item.description + " ")
            .toLowerCase()
  ...
  methods: {
    ...
    deleteEntry(id, index) {
      if (confirm("Do you really want to delete it?")) {
        var app = this;
        axios
          .delete("/auth/specializedministry/delete/" + id)
          .then(function(resp) {
            var index = app.datas
              .map(x => {
                return x.id;
