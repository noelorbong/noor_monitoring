    ...
    <div class="col-sm-6 col-xl-3">
      <div class="card">
        <div class="card-header">
          <i class="nav-icon fa fa-fort-awesome"></i> Ministry
          <router-link
            v-if="userId != 1"
            :to="{name: 'SpecializedMinistryEdit' ,params: {id: userId}}"
            style="float:right;"
        ...
        <div class="card-body text-center">
          <u>{{ data.specializedministry_name || ''}}</u>
          <br />
          <span style="font-weight: bold;">Tribe Name</span>
          <br />
          <br />
          <u>{{data.specializedministry_member || ''}}</u>
          <br />
              ...
              <tbody>
                <tr v-for="(member) in searchResult" :key="member.id">
                  ...
                  <td
                    @click="goToProfile(member.id)"
                    style="cursor:pointer"
                  >{{ member.specializedministry_name == null ? " ":member.specializedministry_name }}</td>
                  <td
                    @click="goToProfile(member.id)"
                    style="cursor:pointer"
                  >{{ member.tribe_name == null ? " ":member.tribe_name }}</td>
      ...
      <div class="card" style="margin:0; padding: 0;">
        ...
        <div class="card-body">
          <form autocomplete="off" v-on:submit="saveForm()">
            ...
            <input type="hidden" class="form-control" v-model="member.specializedministry" />
...
export default {
  ...
  methods: {
    ...
    loadActivity() {
      ...
      app.isLoading = true;
      axios
        .get("/auth/specializedministry/profile/" + id)
        .then(function(resp) {
          // console.log(resp.data);
          app.data = resp.data.user;
          app.filename = "specializedministry_" + resp.data.user.specializedministry_name + ".xls";
    ...
    saveData: function() {
      ...
      formData.append("specializedministry_id", app.userId);
      formData.append("member_id", availability);
      axios
        .post("/auth/specializedministry/member/update", formData)
        .then(function(resp) {
          console.log(resp);
    ...
    saveForm() {
      var app = this;
      app.member.specializedministry = app.userId;
      ...
      axios
        ...
        .then(function(resp) {
          ...
          app.member.tribe = 1;
          app.member.specializedministry = 1;
          app.member.label = 0;
      ...
      axios
        ...
        .then(function(resp) {
          ...
          $.each(app.members, function(key, value) {
            if (value.specializedministry) {
              var specializedministry_name = [];
              var specializedministry_split = value.specializedministry.split(",");
              ...
              app.members[key].specializedministry_name = specializedministry_name.join(",");
            }
          });
        })
        .catch(function(resp) {
    ...
    removeEntry(id) {
      ...
      if (confirm("Do you really want to remove it?")) {
        axios
          .delete("/auth/specializedministry/remove/" + id + "/" + app.userId)
          .then(function(resp) {
