            ...
            <tbody>
              <tr v-for="(member, index) in searchResult" :key="member.id">
                ...
                <td
                  @click="goToProfile(member.id)"
                  style="cursor:pointer"
                >{{ member.email_address == null ? " ":member.email_address }}</td>
                <td
                  @click="goToProfile(member.id)"
                  style="cursor:pointer; width: 120px;"
                >{{ member.dob == null ? " ":member.dob }}</td>
                <td
                  @click="goToProfile(member.id)"
                  style="cursor:pointer; width: 120px;"
                ...
                >{{ member.gender == null ? " ":member.gender }}</td>
                ...
                >{{ member.address1 == null ? " ":member.address1+","}} {{ member.address2 == null ? " ":member.address2+","}} {{ member.barangay == null ? " ":member.barangay+"," }} {{ member.municipality == null ? " ":member.municipality+"," }} {{ member.province == null ? " ":member.province+ ","}} {{ member.zipcode == null ? " ":member.zipcode}}</td>
                <td
                  @click="goToProfile(member.id)"
                  style="cursor:pointer;  width: 100px; "
                >{{ member.created_at }}</td>
                <td>
                  <button class="btn btn-sm btn-danger" v-on:click="deleteEntry(member.id)">
