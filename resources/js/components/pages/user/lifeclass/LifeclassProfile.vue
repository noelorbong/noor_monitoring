    ...
    <div class="col-sm-6 col-xl-3">
      <div class="card">
        <div class="card-header">
          <i class="nav-icon fa fa-fort-awesome"></i> Lifeclass
          <router-link
            v-if="userId != 1"
            :to="{name: 'UserLifeclassEdit' ,params: {id: userId}}"
            style="float:right;"
        ...
        <div class="card-body text-center">
          <u>{{ data.lifeclass_name || ''}}</u>
          <br />
          <span style="font-weight: bold;">Tribe Name</span>
          <br />
          <br />
          <u>{{data.lifeclass_member || ''}}</u>
          <br />
              ...
              <tbody>
                <tr v-for="(member) in searchResult" :key="member.id">
                  ...
                  <td
                    @click="goToProfile(member.id)"
                    style="cursor:pointer"
                  >{{ member.lifeclass_name == null ? " ":member.lifeclass_name }}</td>
                  <td
                    @click="goToProfile(member.id)"
                    style="cursor:pointer"
                  >{{ member.tribe_name == null ? " ":member.tribe_name }}</td>
      ...
      <div class="card" style="margin:0; padding: 0;">
        ...
        <div class="card-body">
          <form autocomplete="off" v-on:submit="saveForm()">
            ...
            <input type="hidden" class="form-control" v-model="member.lifeclass" />
...
export default {
  ...
  methods: {
    ...
    loadActivity(tribe_id) {
      let app = this;
      let id = app.$route.params.id;
      app.userId = id;
      app.isLoading = true;
      axios
        .get("/auth/user/lifeclass/profile/"+tribe_id+"/" + id)
        .then(function(resp) {
          // console.log(resp.data);
          app.data = resp.data.user;
          app.filename = "lifeclass_" + resp.data.user.lifeclass_name + ".xls";
          app.reports = resp.data.records;

          app.availability = [];
          app.unavailability = [];
          $.each(resp.data.records, function(key, value) {
            app.unavailability.push(value.id);
    ...
    saveData: function() {
      ...
      formData.append("lifeclass_id", app.userId);
      formData.append("member_id", availability);
      axios
        .post("/auth/user/lifeclass/member/update", formData)
        .then(function(resp) {
          console.log(resp);
          // app.$router.push("/main/activity/list");
          app.loadActivity(app.g_tribe_id);
          app.memberList(app.g_tribe_id);
          app.hide();
        })
        .catch(function(resp) {
          console.log(resp);
    ...
    saveForm() {
      var app = this;
      app.member.lifeclass = app.userId;
      var member = app.member;
      axios
        .post("/auth/user/member/store", member)
        .then(function(resp) {
          app.loadActivity(g_tribe_id);
          app.hideCustom();
    ...
    memberList(tribe_id) {
      ...
      axios
        ...
        .then(function(resp) {
          ...
          //   if (value.lifeclass) {
          //     var lifeclass_name = [];
          //     var lifeclass_split = value.lifeclass.split(",");

          //     $.each(lifeclass_split, function(key1, value1) {
          //       $.each(resp.data.lifeclasses, function(key2, value2) {
          //         if (value1 == value2.value) {
          //           lifeclass_name.push(value2.text);
          //         }
          //       });
          //     });
          //     app.members[key].lifeclass_name = lifeclass_name.join(",");
          //   }
          // });
        })
        .catch(function(resp) {
    ...
    removeEntry(id) {
      ...
      if (confirm("Do you really want to remove it?")) {
        axios
          .delete("/auth/user/lifeclass/remove/" + id + "/" + app.userId)
          .then(function(resp) {
