      ...
      <div class="col-sm-4" v-for="(data) in searchResult" :key="data.id">
        <div class="card">
          ...
          <div class="card-footer">
            ...
            <router-link
              :to="{name: 'UserProgressProfile', params: {id: data.id}}"
            ...
            <!-- <router-link
              v-if="data.id != 1"
              :to="{name: 'ProgressEdit' , params: {id: data.id}}"
...
export default {
  ...
  computed: {
    searchResult: function() {
      ...
      return this.datas.filter(item => {
        return (
          (item.progress_name + " " + item.description + " ")
            .toLowerCase()
  ...
  methods: {
    ...
    app.isLoading = true;
    axios
      .get("/auth/user/progress/list/"+tribe_id)
      .then(function(resp) {
    ...
    //     var app = this;
    //     axios
    //       .delete("/auth/progress/delete/" + id)
    //       .then(function(resp) {
    //         var index = app.datas
    //           .map(x => {
    //             return x.id;
