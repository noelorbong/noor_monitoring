    ...
    <div class="col-sm-6 col-xl-3">
      <div class="card">
        <div class="card-header">
          <i class="nav-icon fa fa-fort-awesome"></i> Progress
          <router-link
            v-if="userId != 1"
            :to="{name: 'UserProgressEdit' ,params: {id: userId}}"
            style="float:right;"
        ...
        <div class="card-body text-center">
          <u>{{ data.progress_name || ''}}</u>
          <br />
          <span style="font-weight: bold;">Tribe Name</span>
          <br />
          <br />
          <u>{{ data.progress_member || ''}}</u>
          <br />
              ...
              <tbody>
                <tr v-for="(member) in searchResult" :key="member.id">
                  ...
                  <td
                    @click="goToProfile(member.id)"
                    style="cursor:pointer"
                  >{{ member.progress_name == null ? " ":member.progress_name }}</td>
                  <td
                    @click="goToProfile(member.id)"
                    style="cursor:pointer"
                  >{{ member.tribe_name == null ? " ":member.tribe_name }}</td>
      ...
      <div class="card" style="margin:0; padding: 0;">
        ...
        <div class="card-body">
          <form autocomplete="off" v-on:submit="saveForm()">
            ...
            <input type="hidden" class="form-control" v-model="member.progress" />
...
export default {
  ...
  methods: {
    ...
    loadActivity(tribe_id) {
      ...
      app.isLoading = true;
      axios
        .get("/auth/user/progress/profile/"+tribe_id+"/" + id)
        .then(function(resp) {
          // console.log(resp.data);
          app.data = resp.data.user;
          app.filename = "progress_" + resp.data.user.progress_name + ".xls";
    ...
    saveData: function() {
      ...
      formData.append("progress_id", app.userId);
      formData.append("member_id", availability);
      axios
        .post("/auth/user/progress/member/update", formData)
        .then(function(resp) {
          console.log(resp);
    ...
    saveForm() {
      var app = this;
      app.member.progress = app.userId;
    ...
    axios
      ...
      .then(function(resp) {
        app.members = resp.data.records;
          ...
          //   if (value.progress) {
          //     var progress_name = [];
          //     var progress_split = value.progress.split(",");

          //     $.each(progress_split, function(key1, value1) {
          //       $.each(resp.data.categories, function(key2, value2) {
          //         if (value1 == value2.value) {
          //           progress_name.push(value2.text);
          //         }
          //       });
          //     });
          //     app.members[key].progress_name = progress_name.join(",");
          //   }
          // });
        })
        .catch(function(resp) {
    ...
    removeEntry(id) {
      ...
      if (confirm("Do you really want to remove it?")) {
        axios
          .delete("/auth/user/progress/remove/" + id + "/" + app.userId)
          .then(function(resp) {
