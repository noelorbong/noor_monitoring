  ...
  <div>
    <div class="card">
      <div class="card-header">
        <router-link
          :to="{name: 'MemberProfile', params: {id: userId}}"
          style="float:right;"
          class="btn btn-sm btn-warning"
        >
          <i class="fa fa-arrow-left"></i>
        </router-link>
        <i class="nav-icon fa fa-pencil"></i> Profile Edit
      </div>
  ...
  mounted() {
    ...
    axios
      ...
      .then(function(resp) {
        ...
        app.member = resp.data.user;
        app.url = "/uploads/" + resp.data.user.photo;
        app.photo_name = resp.data.user.photo;

        var category_split = resp.data.user.category.split(",");
        $.each(category_split, function(key, value) {
          $.each(app.options, function(key1, value1) {
            if (value1.value == value) {
              app.items.push(value1);
            }
          });
        });
      })
  ...
  methods: {
    saveForm() {
      if (confirm("Do you really want to Update the Profile?")) {
        ...
        formData.append("id", app.userId);
        formData.append("photo", app.member.photo);
        formData.append("photo_name", app.photo_name);
        ...
        axios
          .post("/auth/user/member/update", formData)
          .then(function(resp) {
            // console.log(resp);
            app.$router.push("/user/member/profile/" + app.userId);
          })
          .catch(function(resp) {
            console.log(resp);
