  ...
  <div class="vld-parent">
    ...
    <div class="card">
      <div class="card-header">
        ...
        <router-link
          :to="{name: 'TribeactivityCreate',  params: {id: mainId}}"
            ...
            <tbody>
              <tr v-for="(data, index) in searchResult" :key="data.id">
                ...
                <td style="width: 120px;">
                  <router-link
                    :to="{name: 'TribeactivityProfileEdit', params: {id: data.id}}"
                    style="float:right;"
...
<script>
import LineExample from "./TribelineExample.vue";
...
export default {
  ...
  created: function() {
    ...
    axios
      ...
      .catch(function(resp) {
        console.log(resp);
        alert("Could not load Tribe Activity");
        app.isLoading = false;
      });
  },
  computed: {
  ...
  methods: {
    ...
    deleteEntry(id, index) {
      if (confirm("Do you really want to delete it?")) {
        var app = this;
        axios
          .delete("/auth/tribeactivity/delete/" + id)
          .then(function(resp) {
            app.datas.splice(index, 1);
          })
          .catch(function(resp) {
            alert("Could not delete Tribe Activity");
          });
      }
    },
    onCancel() {
