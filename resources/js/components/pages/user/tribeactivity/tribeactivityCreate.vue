    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:30px; margin:0;">
        <form autocomplete="off" v-on:submit="saveForm()">
          <div class="form-row">
            <input type="hidden" v-model="data.main_tribeactivity_id" />
            <input type="hidden" v-model="data.tribe_id" />
...
export default {
  mounted() {
    ...
    app.userId = id;
    axios
      .get("/auth/tribeactivity/main/edit/" + id)
      .then(function(resp) {
        app.data.main_tribeactivity_id = resp.data.record.id;
        app.data.title = resp.data.record.title;
        app.data.tribe_id = resp.data.record.tribe_id;
      })
      .catch(function() {
  ...
  methods: {
    saveForm() {
      ...
      var data = app.data;
      axios
        .post("/auth/tribeactivity/store", data)
        .then(function(resp) {
          console.log(resp);
          app.$router.push("/user/tribeactivity/list/" + app.userId);
        })
        .catch(function(resp) {
          console.log(resp);
          alert("Could not create tribe activity");
        });
    }
  }
};
